...
const updateFavorite = async (req, res) => {
    ...
    try {
        ...
        if (updatedRows && updatedRows[0] > 0) {
            res.status(200).send({ message: `${updatedRows[0]} medico(s) com informação de favorito atualizada com sucesso` })
        } else {
...
const deleteDoctor = async (req, res) => {
    ...
    try {
        ...
        if (rowsDeleted) {
            res.status(200).send({ message: `Médico(s) deletado(s) com sucesso` })
        } else {
